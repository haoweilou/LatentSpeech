...
encoder = loadModel(encoder,f"flow_rvq_encoder_{num}","./model/",strict=True)
decoder = loadModel(decoder,f"flow_rvq_decoder_{num}","./model/",strict=True)
vq_layer = loadModel(vq_layer,f"flow_rvq_vq_{num}","./model/",strict=True)
...
with torch.no_grad():
    for audio in tqdm(loader):
        ...
        draw_wave(a[0][0].to("cpu"),f"flow_rvq")
        save_audio(a[0].to("cpu"),48000,f"flow_rvq")
