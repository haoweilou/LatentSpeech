...
loss_log = pd.DataFrame({"total_loss":[],"vq_loss":[],"audio_loss":[],"feature_loss":[]})
...
for epoch in range(epochs):
    ...
    for audio in tqdm(loader):
        ...
        y,audio_loss,vq_loss, feature_loss = model(audio)

        loss = vq_loss + audio_loss + feature_loss
        loss_val += loss.item()
    ...
    print(f"Epoch: {epoch} Audio Loss: {audio_loss_/len(loader):.03f} VQ Loss: {vq_loss_/len(loader):.03f} Feature Loss: {feature_loss_/len(loader):.03f} Total: {loss_val/len(loader):.03f}")
    
    if epoch % 50 == 0:
        saveModel(model,f"{model_name}_{epoch}","./model/")

    loss_log.loc[len(loss_log.index)] = [loss_val/len(loader),vq_loss_/len(loader),audio_loss_/len(loader),feature_loss_/len(loader)]
