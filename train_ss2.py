...
loss_log = pd.DataFrame({"total_loss":[],"mse_loss":[],"duration_loss":[],"reconstruct_loss":[]})
...
for epoch in range(0,1001):
    ...
    for i,(text_batch,audio_batch) in enumerate(tqdm(loader)):
        ...
        (y_gen, *_), *_, (attn_gen, *_) = model(x, s, x_lens,y_lens=y_lens, gen=True, noise_scale=1, length_scale=1,g=speaker)
        ...
        reconstruct_loss_ += l_reconscturct.item()
    print(f"Epoch: {epoch} MLE Loss: {mle_loss_/len(loader):.03f} Duration Loss: {duration_loss_/len(loader):.03f} Reconstruct Loss: {reconstruct_loss_/len(loader):.03f} Total: {total_loss/len(loader):.03f}")
    if epoch % 100 == 0:
        saveModel(model,f"{modelname}_{epoch}","./model/")
    loss_log.loc[len(loss_log.index)] = [total_loss/len(loader),mle_loss_/len(loader),duration_loss_/len(loader),reconstruct_loss_/len(loader)]
