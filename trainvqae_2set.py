...
loader = DataLoader(dataset,batch_size=16,collate_fn=dataset1.collate,drop_last=True,shuffle=True)
epochs = 401
model_name = "vqae16"

loss_log = pd.read_csv(f"./log/loss_{model_name}", index_col=0)
print(loss_log)
model = loadModel(model,f"{model_name}_30","./model")
